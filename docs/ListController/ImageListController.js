...
export class ImageListController extends ListController {
    constructor(selector) {
        super(selector, 'image');

        WebOBSData.on('imageAdded', this.onImageAdded.bind(this));
    }
    ...
    createItemElement(item) {
        const removeButton = util.newElm({
            ...
            onclick: evt => WebOBSData.remove(this.type, this.getIndexFromElement(evt.target))
        });
